...
export function VehicleForm({ vehicle, onSubmit, isSubmitting }: VehicleFormProps) {
  ...
  const handleVehiclePhotoReorder = (fromIndex: number, toIndex: number) => {
    ...
    setVehiclePhotos(prev => reorderArray(prev));
    setVehiclePhotoPreviews(prev => reorderArray(prev));
  };

  const handleDetailPhotoReorder = (fromIndex: number, toIndex: number) => {
    const reorderArray = <T,>(arr: T[]): T[] => {
      const result = [...arr];
      const [removed] = result.splice(fromIndex, 1);
      result.splice(toIndex, 0, removed);
    ...
    setDetailPhotos(prev => reorderArray(prev));
